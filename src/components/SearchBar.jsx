...
export const SearchBar = ({ className = '', placeholder = DEFAULT_PLACEHOLDER }) => {
  const { searchQuery, setSearchQuery } = useSearchStore();
  const navigate = useNavigate();

  const [isOpen, setIsOpen] = useState(false);
  const [isExpanded, setIsExpanded] = useState(false); // Toggle only on laptop (md:)
    ...
    <div ref={wrapperRef} className={cn('relative', className)} style={{ minWidth: 0 }}>
      <div
        className={cn(
          ...
          isExpanded && 'md:w-full md:max-w-md' // Expanded on laptop
        )}
      >
        <div
          className={cn(
