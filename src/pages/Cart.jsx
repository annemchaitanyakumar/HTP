...
export default function Cart() {
  ...
  const calculateTotalContainerFee = () => {
    return items.reduce((total, item) => total + (item.containerFee || 0), 0);
  };
  ...
  const calculateShipping = () => {
    ...
    return totalWithoutContainer >= 500 ? 0 : taxInfo.shippingCharges;
  };
  
  const subtotal = calculateSubtotal();
  const totalContainerFee = calculateTotalContainerFee();
  const tax = (subtotal * (taxInfo.gstPercentage || 0)) / 100;
  const shipping = calculateShipping();
  // Include container fee in total calculation
  const total = subtotal + totalContainerFee + shipping + tax;
  
  // Calculate how much more needed for free shipping
  const remainingForFreeShipping = Math.max(0, 500 - (subtotal - totalContainerFee));
                          ...
                          <div className="flex items-start gap-4 w-full lg:w-1/2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="font-bold text-primary text-sm sm:text-base md:text-lg">
                                ...
                                {item.containerFee ? ` + ₹${item.containerFee} (container)` : ''}
                              </p>
                            </div>
                          </div>
